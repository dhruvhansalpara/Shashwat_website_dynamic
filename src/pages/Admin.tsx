...
import { fetchDestinations, saveDestination, deleteDestination, fetchPackages, savePackage, deletePackage } from '../utils/api';
...
export default function AdminPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [d, p] = await Promise.all([fetchDestinations(), fetchPackages()]);
      setDestinations(d);
      setPackages(p);
    } catch (err) {
      console.error(err);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          ...
          <div className="mt-4 md:mt-0 flex gap-2">
            <button 
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'destinations' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
            >
              Destinations
            </button>
            <button 
              onClick={() => setActiveTab('packages')}
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'packages' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
            >
              Packages
            </button>
          </div>
